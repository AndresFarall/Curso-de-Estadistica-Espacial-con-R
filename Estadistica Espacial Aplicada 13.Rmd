...
visible.caba<-crop(visible,caballito)
...
visible.luga<-crop(visible,lugano)
plotRGB(visible.luga,scale=1)
plot(lugano,border="yellow",add=T,col=NA,lwd=3)

```
Busco las celdas com mayor relacion de verde

```{r}
prom.RB<-mean(visible.caba$Red,visible.caba$Blue)
rela.G.RB<-visible.caba$Green/prom.RB
hist(values(rela.G.RB),100)
...
cumplen[!cuales]<-NA
plotRGB(cumplen,scale=1)
plot(caballito,border="yellow",add=T,col=NA,lwd=3)
plotRGB(nocumplen,scale=1)
plot(caballito,border="yellow",add=T,col=NA,lwd=3)
...
```{r}
cant<-6
visible.caba.mat<-values(visible.caba) # raster como matriz
set.seed(1)
...
values(visible.caba.clust)<-kmncluster$cluster
...
```{r}
cant<-10
visible.luga.mat<-values(visible.luga) # raster como matriz
set.seed(1)
...
values(visible.luga.clust)<-kmncluster$cluster
mycolor <- c("#fef65b","#ff0000", "#daa520","#0000ff","#0000ff","#00ff00","#cbbeb5",
             "#c3ff5b", "#ff7373", "#00ff00", "#808080")
#par(mfrow=c(1,2))
#plotRGB(visible.caba,scale=1)
...
visible.vers<-crop(visible,versalles)
plotRGB(visible.vers,scale=1)
...
```{r}
cant<-6
visible.vers.mat<-values(visible.vers) # raster como matriz
set.seed(1)
...
values(visible.vers.clust)<-kmncluster$cluster
mycolor <- c("#fef65b","#ff0000", "#daa520","#0000ff","#0000ff","#00ff00","#cbbeb5",
             "#c3ff5b", "#ff7373", "#00ff00", "#808080")
#par(mfrow=c(1,2))
#plotRGB(visible.caba,scale=1)
...
Extraccion de valores de los deptos
...
rojo.selec<-selec$Red
verdes.total<-terra::values(visible$Green)
verdes.selec<-selec$Green
azul.total<-terra::values(visible$Blue)
azul.selec<-selec$Blue

#
plot(density(rojo.total),col="red")
lines(density(rojo.selec),col="black")
plot(density(verdes.total),col="green")
lines(density(verdes.selec),col="black")
plot(density(azul.total),col="blue")
lines(density(azul.selec),col="black")
# Espacios verdes
...
#
rojo.selec<-selec.parques$Red
verdes.selec<-selec.parques$Green
azul.selec<-selec.parques$Blue
#
plot(density(rojo.total),col="red")
lines(density(rojo.selec),col="black")
plot(density(verdes.total),col="green")
