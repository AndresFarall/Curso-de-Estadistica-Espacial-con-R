...
pplot(calles)
plot(avenidas)
plot(calles.comp[1:2000,"tipo_c","nom_mapa","tipo_c","long"])
leaflet(avenidas) %>% addTiles() %>% addPolylines()
leaflet(rivadavia) %>% addTiles() %>% addPolylines()
# Que avenidas cortan rivadavia
cortan_riv<-st_intersection(avenidas[-cuales,],rivadavia)
cortan_riv
leaflet(cortan_riv) %>% addTiles() %>% addCircleMarkers()
tocan<-st_intersects(avenidas[-cuales,],cortan_riv)
cuales.tocan<-unlist(tocan)
plot(avenidas[-cuales,][cuales.tocan,])
```
...
# with ggplot
ggplot() + geom_sf(data = deptos.sf, size = 0.3) + geom_sf(data = barrios, aes(fill = BARRIO), alpha = 0.5)
ggplot() + geom_sf(data = deptos.sf, size = 0.3) + geom_sf(data = barrios, alpha = 0.5) 
```
